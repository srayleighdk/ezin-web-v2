...
import { setAuth, setCart } from '../../store/actions';
import { toggleLoginModal, toggleRegisterModal } from '../../store/modal/actions';
...
export function ProtectedRoute(Component) {
  return (props) => {
    ...
    if (!isAuthenticated && window.location.pathname !== '/login') {
      ...
      // return null;
      return (
        <div>Vui lòng <a className="text-link" href="#" onClick={() => dispatch(toggleLoginModal())}>đăng nhập</a> hoặc <a className="text-link" href="#" onClick={() => dispatch(toggleRegisterModal())}>đăng ký</a> tài khoản Ezin để tiếp tục</div>
      )
    }
