import { Button, Col, Form, Input, message, Row, Upload, Typography, Checkbox } from 'antd';
...
import { getPartnerBranches, getCities, getWards, getDistricts } from '../../../pages/api';
...
export default function Third({ onPrevStep, onNextStep, cardInfo }) {
  ...
  const [minStartDate, setMinStartDate] = useState(nextDay.clone())

  const maxStartDate = moment(minStartDate).add(7, 'days');
  const getInitData = async () => {
    const [res1] = await Promise.all([
      getCities(),
    ]);
    setCities(res1?.data?.data?.map(e => ({ label: e.name_with_type, value: e.code, group: e.type })))
    if (cardInfo?.package_id?.type == 'COMBO') {
      const res2 = await getPartnerBranches(cardInfo?.package_id?.product_id?.agency_id);
      setPartnerBranches(res2?.data?.data?.map(e => ({ label: `${e.name} - ${e.address}`, value: e._id })))
    }
  }
  ...
  const loadDistricts = async (value, clear = false) => {
    console.log('typeof value loadDistricts', typeof value)
    console.log('value loadDistricts', value)
  ...
  const loadWards = async (value, clear = false) => {
    console.log('typeof value loadWards', typeof value)
  ...
  const onFinish = (values) => {
    if (cardInfo.package_id.type == 'COMBO' && !tosChecked) {
      message.error(`Vui lòng đọc và đồng ý với Quy định Câu lạc bộ của CITIGYM`);
      return
    }
    let { dob, ...rest } = values;
    ...
    const partner_starttime = moment.isMoment(values.partner_starttime) ? moment(values.partner_starttime).format('DD/MM/YYYY') : values.partner_starttime;

    let body = {
      ...rest,
      dob: dateOfBirth,
      valid_from: valid_from,
      partner_starttime: partner_starttime,
      main_account: true
    };
  ...
  const handleSelectValidDate = (time) => {
    ...
    setMinStartDate(moment(time)?.clone())
    form.setFieldsValue({
  ...
  const handleSelectStartDate = (time) => {
    setOpenStartDate(false);
    form.setFieldsValue({
      partner_starttime: moment(time).format('DD/MM/YYYY')
    });
  }
      ...
      >
        <div className="p-2 ezin-card">
          ...
          <Row gutter={[8, 8]} className="mt-3">
            ...
            {cardInfo?.package_id?.type == 'COMBO' && (
              <>
                <Col xs={{ span: 24 }} md={{ span: 24 }}>
                  <hr />
                  <h3>Thông tin dịch vụ Premium</h3>
                </Col>
                <Col xs={{ span: 24 }} md={{ span: 24 }}>
                  <Field type="select"
                    name="partner_branch"
                    required={true}
                    title="Vui lòng chọn CLB bạn muốn quy đổi dịch vụ"
                    datasource={partnerBranches} />
                </Col>
                <Col xs={{ span: 24 }} md={{ span: 24 }}>
                  <Field type="date"
                    label="Vui lòng chọn ngày dự kiến bắt đầu tập luyện"
                    required={true}
                    handleOpenDate={handleOpenStartDate}
                    initialValue={isMobile ? minStartDate?.format('DD/MM/YYYY') : minStartDate}
                    name="partner_starttime"
                    defaultValue={minStartDate}
                    title="Vui lòng chọn ngày dự kiến bắt đầu tập luyện"
                    onChange={handleFieldChange}
                    minDate={new Date(minStartDate.year(), minStartDate.month(), minStartDate.date())}
                    maxDate={new Date(maxStartDate.year(), maxStartDate.month(), maxStartDate.date())}
                    onFieldChange={handleFieldChange}
                    withPortal={true}
                  />
                </Col>
                <Col xs={{ span: 24 }} md={{ span: 24 }}>
                  <Checkbox checked={tosChecked} onChange={() => setTosChecked(!tosChecked)}>
                    Tôi đã đọc và hiểu rõ <a className="font-weight-bold font-style-normal text-deco-underline" href='https://ms.citigym.com.vn/quydinhcaulacbo' target="_blank">Quy định Câu lạc bộ</a> của CITIGYM, xác
                    nhận đồng ý tham gia chương trình và cam kết các thông
        ...
        <div className="text-center">
          <ButtonEzin types="default" htmlType="button" className="btn-kichhoat bg-danger" onClick={onReset}>
            Làm lại
          </ButtonEzin>
        </div>
      </Form>
