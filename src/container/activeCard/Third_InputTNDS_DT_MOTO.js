...
export default function ThirdMOTO({ onPrevStep, onNextStep, cardInfo }) {
  ...
  const loadDistricts1 = async (value) => {
    const res = await getDistricts(value);
    setDistricts1(
      res?.data?.data?.map((e) => ({ label: e.name_with_type, value: e.code }))
    );
  };
  const loadWards1 = async (value) => {
    const res = await getWards(value);
    setWards1(
      res?.data?.data?.map((e) => ({ label: e.name_with_type, value: e.code }))
    );
  ...
  const onChangeLicenseType = (value) => {
    ...
    if (value == "license") {
      ...
      cardInfo.engineNumber = "";
      cardInfo.chassisNumber = "";
    } else if (value == "engine") {
      form.setFieldsValue({ licenseNumber: "" });
      ...
      >
        <div className="p-2 ezin-card">
          ...
          <Row gutter={[8, 8]} className="mt-3">
            ...
            <Col span={24}>
              {cardInfo.package_id.program_id.capacity_options &&
                cardInfo.package_id.program_id.capacity_options.length > 1 && (
                  <Col xs={{ span: 24 }} md={{ span: 24 }}>
                    ...
                    >
                      ...
                      >
                        {cardInfo.package_id.program_id.capacity_options.map(
                          (e) => (
                            <Radio value={e.value} style={{ display: "block" }}>
                              {e.label}
                            </Radio>
                          )
                        )}
