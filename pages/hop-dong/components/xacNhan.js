    ...
    const renderContent = (type) => {
        ...
        if (showMore) {
            tableDataInfo = tableDataInfo.concat([
                ['Nhà cung cấp', 'PVI'],
                ['Hiệu lực từ ngày', moment(submitData?.valid_from, 'DD/MM/YYYY').format('DD/MM/YYYY')],
                ['Giá trị bảo hiểm', `${data?.program_id?.total_fee}`],
                ['Thời hạn bảo hiểm', `${data?.period?.toString()?.padStart(2, '0')} năm`],
        ...
        if (['OTO', 'XE_MAY'].includes(type)) {
            ...
            return (<>
                ...
                <Button type="link" icon={showMore ? <CaretUpOutlined /> : <CaretDownOutlined/>} className="mb-1 xac-nhan__btn" onClick={() => setShowMore(!showMore)} >{!showMore ? "Xem thêm" : "Ẩn bớt"}</Button>
                <h3>Người nhận bảo hiểm</h3>
                <TableInfo tableData={dataReceiver} />
                <Button type="link" icon={showMoreReceiver ? <CaretUpOutlined /> : <CaretDownOutlined/>} className="mb-1 xac-nhan__btn" onClick={() => setShowMoreReceiver(!showMoreReceiver)} >{!showMoreReceiver ? "Xem thêm" : "Ẩn bớt"}</Button>
            </>);
        }
        return (<>
            <TableInfo tableData={tableDataInfo} />
    ...
    return (<div>
        ...
        <div>
            <div className="d-flex hopdong__xacnhan">
                <Checkbox className="hopdong__xacnhan__checkbox" checked={tos} onChange={(e) => setTos(e.target.checked)}>Tôi đã đọc và đồng ý với các điều kiện và điều khoản về <a className="text-link hopdong__xacnhan__audio" href={`#`} onClick={() => setVisible(true)}>Quy tắc bảo hiểm áp dụng</a></Checkbox>
            </div>
            {
            ...
            <Modal
                title="Quy tắc bảo hiểm"
                visible={visible}
                onOk={() => { setTos(true); setVisible(false) }}
            ...
            >
                <div
                    ...
                    dangerouslySetInnerHTML={{ __html: data?.product_id?.rule_content }}></div>
            </Modal>
            {visibleAudio ?
                <Modal
                    title="Quy tắc bảo hiểm"
