...
export default function ActivateForm({ data, initData, onNext }) {
    ...
    useEffect(async() => {
        if(initData) {
            ...
            setDistricts1(resDistricts1?.data?.data?.map(e => ({ label: e.name_with_type, value: e.code })))
            setWards1(resWards1?.data?.data?.map(e => ({ label: e.name_with_type, value: e.code })))
        }
    }, [initData])
    ...
    const onFinish = (values) => {
        console.log('value angia', values)
        onNext(values);
    };
    ...
    return (<div>
        ...
        >
            <Row gutter={[8, 8]}>
                ...
                <Col span={24}>
                    <p className="ml-0 mb-0 text-11">Chọn người được bảo hiểm (hoặc nhấn dấu + để thêm mới)</p>
                ...
                <Col xs={{ span: 24 }} md={{ span: 8 }}>
                    <Field type="select" groups={{ 'thanh-pho': 'Thành phố', 'tinh': 'Tỉnh' }} required={true} name="city_ben" title="Tỉnh thành" datasource={cities1} onChange={(value) => loadDistricts1(value, true)} />
                </Col>
                <Col xs={{ span: 24 }} md={{ span: 8 }}>
                ...
                <Col xs={{ span: 24 }} md={{ span: 24 }}>
                    <Field name="address_ben" type="text" title="Địa chỉ được bảo hiểm" required={true} />
                </Col>
